...
export const api = {
  async login(identifier: string, password: string): Promise<LoginResponse> {
    ...
    if (!res.ok) {
      throw new Error(data?.message || (data as any)?.error || '登录失败')
    }
    return data
  },
  ...
  async verifyOTP(userID: string, otpCode: string): Promise<OTPResponse> {
    ...
    const data: OTPResponse = await res.json().catch(() => ({}))
    if (!res.ok) {
      throw new Error(data?.message || (data as any)?.error || 'OTP验证失败')
    }
    return data
  },
  ...
  async completeRegistration(
    ...
    const data: OTPResponse = await res.json().catch(() => ({}))
    if (!res.ok) {
      throw new Error(data?.message || (data as any)?.error || '注册完成失败')
    }
    return data
  },
